...
import json
import re
def get_worldchrman(pm):
    address = pm.base_address + re.search(rb'\x48\x8B\x05....\x48\x85\xC0\x74\x0F\x48\x39\x88', pm.read_bytes(pm.base_address, pymem.process.module_from_name(pm.process_handle, 'eldenring.exe').SizeOfImage)).start()
    return address+pm.read_int(address+3)+7
# from importlib import import_module


def get_eventflagman(pm):
    address=pm.base_address + re.search(rb'\x48\x8B\x3D....\x48\x85\xFF..\x32\xC0\xE9', pm.read_bytes(pm.base_address, pymem.process.module_from_name(pm.process_handle, 'eldenring.exe').SizeOfImage)).start()
    return address+pm.read_int(address+3)+7
...
def get_const_by_name(pm, name):
    if(name=='worldchrman'):
        return get_worldchrman(pm)
    elif(name=='eventflagman'):
        return get_eventflagman(pm)
